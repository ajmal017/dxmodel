  ...
  class << self
    ...
    def process_enter_signals date
      Trade.enter_signaled.each do |trade|
        ...
        if stock_date.present?
          trade.enter_usd_fx_rate = FxRate.where('date >= ?', date).order('date ASC').first.send('usd' + trade.stock.currency.downcase)
          enter_local_budget = MAX_PER_ENTRY * trade.enter_usd_fx_rate
    ...
    def process_exit_signals date
      Trade.exit_signaled.each do |trade|
        stock_date = StockDate.where('stock_id = ? and date > ?', trade.stock_id, date).order('date ASC').first
        if stock_date.present?
          trade.exit_usd_fx_rate = FxRate.where('date >= ?', date).order('date ASC').first.send('usd' + trade.stock.currency.downcase)
